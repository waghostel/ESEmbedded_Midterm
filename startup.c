...
void set_sysclk_pll(void)
{
	...
	//set pll
	CLEAR_BIT(RCC_BASE + RCC_PLLCFGR_OFFSET, PLLSRC_BIT); //use HSL for PLL source
	...
	*/
	WRITE_BITS(RCC_BASE + RCC_PLLCFGR_OFFSET, PLLP_1_BIT, PLLP_0_BIT, 0b00); //0b01=4->0b00:x2 
	WRITE_BITS(RCC_BASE + RCC_PLLCFGR_OFFSET, PLLN_8_BIT, PLLN_0_BIT, 336);//84->336: x4
	WRITE_BITS(RCC_BASE + RCC_PLLCFGR_OFFSET, PLLM_5_BIT, PLLN_0_BIT, 2);//4

	//enable pll: set bit 24 to 1
	...
	//wait
	 ...
	 || (READ_BIT(RCC_BASE + RCC_CFGR_OFFSET, SWS_0_BIT) !=0) //make this cfgr
